...
(defn resolve-group [group]
  (deref (resolve (create-symbol "group" group))))

(defn generate-select[group table columns]
  `(defn ~(create-symbol "select" group table) []
     (let [~(create-symbol table "fields-var") ~columns]
       (select ~(name table) (~'fields ~@columns)))))

(defn generate-field-var [user table columns]
  (intern *ns* (create-symbol user table "fields-var") columns))

(defn merge-groups [groups]
  (apply merge-with (comp #(if (some #{:all} %) [:all] (vec %)) concat) groups))

(defn find-within-namespace [user]
  (let [p (->> "-.+-fields-var" (str user) clojure.string/lower-case re-pattern)]
    (filter (fn [[k v]] (->> k name (re-matches p) nil? not)) (ns-map *ns*))))

(defmacro group [name & body]
  (let [group-map (->> body
                       (partition 3)
                       (mapcat (fn [[table _ columns]] [(keyword table) (mapv keyword columns)]))
                       (apply hash-map))]
    `(do
       ~@(map (fn [[table columns]] (generate-select name table columns)) group-map)
       (def ~(create-symbol "group" name) ~group-map))))

(defmacro belongs-to [& groups]
  `(do ~@(map (fn [[table column]] `(generate-field-var ~'current-user-name ~table ~column)) 
              (merge-groups (map resolve-group groups)))))

(defmacro user [name & body]
  `(let [~(symbol "current-user-name") ~(str name)] ~@body))

(defmacro with-user [user-name & body]
  `(let ~(->> user-name
              find-within-namespace
              (mapcat (fn [[k v]] [(-> k name (clojure.string/split #"-" 2) second symbol) (deref v)]))
              vec) 
     ~@body))
